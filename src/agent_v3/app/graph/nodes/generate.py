# from agent_v3.app.models import chat

# SYS = "You are a precise assistant. Use evidence and cite [KB:file]."


# def generate(state):
#     user = next((m for m in reversed(state.get("messages", [])) if m.get("role") == "user"), {"content": ""})
#     ev = state.get("evidence", []) or []
#     ctx = "\n".join([f"- {e.get('source', '')}: {e.get('content', '')}" for e in ev])
#     out = chat([{"role": "system", "content": SYS}, {"role": "user", "content": f"""{user["content"]}\n\nEvidence:\n{ctx}"""}], 0.2, 800)
#     st = dict(state)
#     st["answer"] = out
#     return st
